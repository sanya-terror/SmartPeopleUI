<?php
	...
	$max_image_size = 3800; //Maximum image size (height and width)
	$thumb_prefix = "thumb_"; //Normal thumb Prefix
	$destination_folder	= '../../view/avatars/'; //upload directory ends with / (slash)
	...
	if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
		...
		if($image_res){
			...
			$new_file_name = $filename . '.' . $image_extension;
			...
			if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality)){
				...
				echo '<img src="../avatars/'.$thumb_prefix . $new_file_name.'" alt="Thumbnail">';
				echo "<br />";
				echo "</div>";
			}
	...
	class uploader extends mainprocessing {
		public function __construct(){
			$this->login = $_SESSION['login'];
			$this->avatar = $_SESSION['new_file_name'];
			$connect = $this->ConnectDB();
			$result = $connect->query("UPDATE users SET avatar = '$this->avatar' WHERE login = '$this->login'");
		}
	}
