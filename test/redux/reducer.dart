...
class ReducerTests {
  static run() {
    group('Redux combine reducers', () {
      test('Should return a composite reducer', () {
        Reducer reducer1 = (State state, Action action) =>
            action.type == FIRST_ACTION
                ? (new State.from(state)..['reducer1'] = 'passed')
                : state;

        Reducer reducer2 = (State state, Action action) =>
            action.type == FIRST_ACTION
                ? (new State.from(state)..['reducer2'] = 'passed')
                : state;
      ...
      test(
          'Should return a composite reducer that maps the state keys to given reducers',
          () {
        Reducer reducer1 = (State state, Action action) =>
            action.type == FIRST_ACTION
                ? (new State.from(state)..['reducer1'] = 'passed')
        ...
        expect(combined(State.emptyState, new Action(FIRST_ACTION)),
            {'reducer1': 'passed'});
        expect(combined(State.emptyState, new Action(SECOND_ACTION)),
            {'reducer2': 'passed'});
      });

      test(
          'Should return initial state if no reducers passed to compose function',
          () {
