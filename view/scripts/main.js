...
//Код отвечающий за проверку паролей на совпадение
    ...
    $("input", name).blur(function () {
        rKey_1 = dataValidator($(this), "name", name);
    }); // Проверка имени

    $("input", surname).blur(function () {
        rKey_2 = dataValidator($(this), "name", surname);
    }); // Проверка фамилии

    $("input", email).blur(function () {
        rKey_3 = dataValidator($(this), "mail", email);
    }); // Проверка электронной почты

    $("input", password1).blur(function () {
        rKey_4 = dataValidator($(this), "passw", password1);
    }); // Проверка пароля 1

    $("input", password2).blur(function () {
        rKey_5 = comparePassword($(this), password2);
    }); // Проверка пароля 2

    sex.change(function () {
        rKey_6 = sex.is(':checked');
    }); // Проверка пароля 2
    ...
    $errorSign.each(function (i) {
        if (i == 3) {
            $(this).mouseover(function () {
                $(this).attr("title", pswdMessage($("input", password1).val().length)).tooltip({
                    position: {
                        my: "right+30 bottom-25", at: "center top",
        ...
        $(this).attr("title", messageArray[i]).tooltip({
            position: {
                my: "right+30 bottom-25", at: "center top",
                using: function (position, feedback) {
                    $(this).css(position);
...
//Код отвечающий за проверку форм email и code
            ...
            success: function (data) {
                ...
                if (isMail) {
                    if (!data) {
                        ...
                        $("input[type='text']", restore_form).attr("name", data.input_name).attr("placeholder", data.placeholder).removeClass("email").addClass(data.newClass);
                        $hash.val(data.user_hash);
                        $(".input-submit-wrapper input[type='submit']").attr("value", data.button_value);
                    }
                }
                else {
                    if (data === "0") {
    ...
    rEmail.bind("change", function () {
        rKey_7 = checkFormEntry($(this), "mail", messageArray[2], restore_message[0], restore_message_field);
    }); // Проверка электронной почты
    restore.on("change", ".code", function () {
        rKey_8 = checkFormEntry($(this), "code", messageArray[6], restore_message[1], restore_message_field);
    }); // Проверка поля код
    ...
    function comparePswd(msg1, msg2, help) {
    ...
    new_pswd.bind("blur", function () {
        passKey_1 = changePassword($(this), "passw", pswdMessage($(this).val().length), "", $(".help-new-pswd"));
    }); // Проверка электронной почты
    rpt_pswd.bind("blur", function () {
        passKey_2 = comparePswd(messageArray[4], "", $(".help-rpt-pswd"));
    }); // Проверка электронной почты
