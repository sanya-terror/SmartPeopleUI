  ...
  static run() {
    group('Api action', () {
      test('Should throw error if passed non-supported http method', () {
        expect(() => new ApiAction(ERROR_UNAUTHORIZED, '/test', 'UNKNOWN'), throwsArgumentError);
      });

      test('Should not throw error if supported http method', () {
        expect(() => new ApiAction(ERROR_UNAUTHORIZED, '/test', 'GET'), returnsNormally);
        expect(() => new ApiAction(ERROR_UNAUTHORIZED, '/test', 'POST'), returnsNormally);
        expect(() => new ApiAction(ERROR_UNAUTHORIZED, '/test', 'PUT'), returnsNormally);
        expect(() => new ApiAction(ERROR_UNAUTHORIZED, '/test', 'DELETE'), returnsNormally);
      });
    });
    group('Api action creator', () {
      group('Errors', () {
        ...
        test('Should return bad request error action', () {
          Action result = ApiActionCreator.badRequestAction(new ApiError(400, 'Some error message!'));
        ...
        test('Should return forbidden error action', () {
          Action result = ApiActionCreator.forbiddenAction(new ApiError(403, 'Some error message!'));
        ...
        test('Should return forbidden error clean action', () {
          Action result = ApiActionCreator.forbiddenCleanAction();
          expect(result.type, ERROR_REMOVE_FORBIDDEN);
          expect(result.data, null);
        });

        test('Should return not found error action', () {
          Action result = ApiActionCreator.notFoundAction(new ApiError(404, 'Some error message!'));
        ...
        test('Should return internal server error action', () {
          Action result = ApiActionCreator.internalServerErrorAction(new ApiError(500, 'Some error message!'));
