...
class ApiErrorAction extends Action {
  ApiErrorAction(String type, ApiError error) : super(type, error.data) {
    var allowedActions = [
      BAD_REQUEST_ACTION, UNAUTHORIZED_ACTION, FORBIDDEN_ACTION, NOT_FOUND_ACTION, INTERNAL_SERVER_ERROR_ACTION
    ];
...
class ApiActionCreator {
  ...
  static ApiErrorAction unauthorizedAction(ApiError error) => new ApiErrorAction(UNAUTHORIZED_ACTION, error);

  static ApiErrorAction notFoundAction(ApiError error) => new ApiErrorAction(NOT_FOUND_ACTION, error);

  static ApiErrorAction badRequestAction(ApiError error) => new ApiErrorAction(BAD_REQUEST_ACTION, error);

  static ApiErrorAction forbiddenAction(ApiError error) => new ApiErrorAction(FORBIDDEN_ACTION, error);
